...
class UpTheStairsEnv(gym.Env):
    """Custom Environment that follows gym interface for stair climbing task with discrete actions
    """
    metadata = {'render.modes': ['human']}

    def __init__(self, episode_length, prototypes):
        super(UpTheStairsEnv, self).__init__()
        ...
        self.action_space = spaces.Discrete(n_prototypes)  # n_prototypes is the number of actions
    ...
    def step(self, action):
        ...
        action_values = self.action_prototypes(action)
        jump_ex = JumpRequest()
        jump_ex.magnitude=action_values[0]
        jump_ex.angle=action_values[1]
        self.jumper(jump_ex)
        model_state = self.get_model_state('bd_droid', 'world')
    ...
    def reset(self, seed=0, options=None):
        ...
        return observation, None  # return initial observation
